...
class CacheModel(BaseModel):
    ...
    def get_features_and_compute_correspondence(self, batch, return_histograms=False, window_softargmax=False, pose_align=False):
        ...
        vis = []
        for i in range(sum([len(x) if isinstance(x, list) else 1 for x in permute_list])):
            vis.append(i in batch['mutual_visible_idx'][0])
        
        permutation = permute_indices(permute_list, vis)
