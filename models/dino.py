...
class DINOModel(BaseModel):
    ...
    def __call__(self, sample):
        ...
        )[0].permute(0, 2, 1).reshape(images.shape[0], -1, self.num_patches, self.num_patches)
        source_features = dino_features[:len(source_images)]
        target_features = dino_features[len(source_images):]
